...
plt.savefig(DIR2FIG / "gradient_boosting_data.svg", bbox_inches="tight")
...
def plot_decision_tree_with_residuals(y_train, y_train_pred, y_test_pred):
    ...
    # plot the data
    sns.scatterplot(x=data_train["Feature"], y=y_train, color="black", alpha=0.5, ax=ax)
    # plot the predictions
    line_predictions = ax.plot(data_test["Feature"], y_test_pred, "--")

    # plot the residuals
    for value, true, predicted in zip(data_train["Feature"], y_train, y_train_pred):
        lines_residuals = ax.plot([value, value], [true, predicted], color="red")
...
ax.legend(handles, legend_labels, loc="lower right", prop={"size": 15})
_ = ax.set_title("Decision Tree together \nwith errors on the training set")
plt.savefig(DIR2FIG / "gradient_boosting_fit.svg", bbox_inches="tight")
# %% [markdown]
# ```{tip}
...
ax.legend(handles, legend_labels, loc="lower right", prop={"size": 10})
_ = ax.set_title("Prediction of the initial residuals")
plt.savefig(DIR2FIG / "gradient_boosting_residuals.svg", bbox_inches="tight")

zoom = [-1, 0]
rect = plt.Rectangle(
    (zoom[0], ax.get_ylim()[0]),
    zoom[1] - zoom[0],
...
ax.add_patch(rect)
plt.savefig(
    DIR2FIG / "gradient_boosting_residuals_before_zoom.svg", bbox_inches="tight"
)
# %% [markdown]
...
ax.scatter(sample, target_true, label="Sample of interest", color="tab:orange", s=200)
ax.set_xlim([-1, 0])
ax.legend(loc="lower right", prop={"size": 15})
_ = ax.set_title("Zoom of sample of interest\nin the initial decision tree")
plt.savefig(DIR2FIG / "gradient_boosting_fit_zoom.svg", bbox_inches="tight")
# %% [markdown]
...
plt.savefig(DIR2FIG / "gradient_boosting_residuals_zoom.svg", bbox_inches="tight")
# %% [markdown]
